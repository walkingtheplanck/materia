...
fn aluminum_density() {
    ...
    check_density(
        "aluminum",
        Substance::Crystalline(Crystal::aluminum()),
        2700.0,
        0.55,
    );
}
...
fn check_melting_abs(name: &str, substance: Substance, real: f32, abs_tolerance: f32) {
    ...
    let mp = mat.thermal.melting_point.unwrap_or(f32::NAN);
    let err = (mp - real).abs();
    eprintln!("{name} melting point: {mp:.1} C (real: {real:.1}, abs err: {err:.1} C)",);
    assert!(
        err < abs_tolerance,
...
fn reaction_to_rule_exothermic_adds_heat() {
    ...
    let has_heat = rule.effects.iter().any(|e| {
        matches!(
            e,
...
fn print_material_table() {
    ...
    for (name, mat) in &materials {
        eprintln!(
            "{:<12} {:>8.0} {:>8.0} {:>8.0} {:>8.1} {:>10.0} {:>6.1} {:>5.2}",
            name,
            mat.structural.density,
...
fn iron_yield_strength_order_of_magnitude() {
    ...
    eprintln!(
        "Iron yield strength: {:.0} MPa (real: ~250 MPa for pure iron)",
        ys / 1e6
    );
    assert!(
        ys > 50e6 && ys < 5000e6,
        "Iron yield {:.0} MPa out of range",
        ys / 1e6
    );
}
...
fn gold_is_yellow() {
    ...
    assert!(
        r > g && g > b,
        "Gold color [{r:.2}, {g:.2}, {b:.2}] not yellow"
    );
}
...
fn ethanol_is_condensed_at_room_temp() {
    ...
    assert_ne!(
        mat.phase.state,
        PhaseState::Gas,
        "Ethanol should not be gas (bp=78C)"
    );
}
...
fn water_less_viscous_than_honey_like_materials() {
    ...
    assert!(
        water.phase.relaxation_time < 1.5,
        "Water tau={} too high",
        water.phase.relaxation_time
    );
}
...
fn density_ordering() {
    ...
    assert!(
        fe > st,
        "iron({fe:.0}) should be denser than stone({st:.0})"
    );
